...
def add_order(date, status, shipping_address, price):
    ...
    try:
        cur.execute(
            "INSERT INTO orders (date, status, shipping_address, price, customer_id, canceled) VALUES (%s, %s, %s, %s, %s, %s)", (date, status, shipping_address, price, session.get("user_id"), False)
        )
        conn.commit()
